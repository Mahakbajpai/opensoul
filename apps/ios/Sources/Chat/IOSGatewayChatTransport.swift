...
struct IOSGatewayChatTransport: OpenSoulChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenSoulChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenSoulChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenSoulChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenSoulChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenSoulChatAttachmentPayload]) async throws -> OpenSoulChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenSoulChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(OpenSoulChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(OpenSoulGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenSoulGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenSoulChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenSoulAgentEventPayload.self)
                        {
