...
private let chatUILogger = Logger(subsystem: "ai.opensoul", category: "OpenSoulChatUI")

@MainActor
@Observable
public final class OpenSoulChatViewModel {
    public private(set) var messages: [OpenSoulChatMessage] = []
    ...
    public var attachments: [OpenSoulPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenSoulChatPendingToolCall] = []
    public private(set) var sessions: [OpenSoulChatSessionEntry] = []
    private let transport: any OpenSoulChatTransport
    ...
    private var pendingToolCallsById: [String: OpenSoulChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpenSoulChatTransport) {
    ...
    public var sessionChoices: [OpenSoulChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [OpenSoulChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [OpenSoulChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenSoulPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenSoulChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenSoulChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [OpenSoulChatMessage]) -> [OpenSoulChatMessage] {
        var result: [OpenSoulChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenSoulChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenSoulChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> OpenSoulChatAttachmentPayload in
            OpenSoulChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenSoulChatSessionEntry {
        OpenSoulChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpenSoulChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenSoulChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenSoulAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenSoulChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenSoulPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
