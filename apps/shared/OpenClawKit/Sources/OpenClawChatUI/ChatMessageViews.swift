...
struct ChatMessageBubble: View {
    let message: OpenSoulChatMessage
    let style: OpenSoulChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OpenSoulChatTheme.userText : OpenSoulChatTheme.assistantText
    ...
    private var inlineAttachments: [OpenSoulChatMessageContent] {
    ...
    private var toolCalls: [OpenSoulChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OpenSoulChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OpenSoulChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OpenSoulChatTheme.onboardingAssistantBubble
        }
        return OpenSoulChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OpenSoulChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OpenSoulChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OpenSoulChatTheme.userText : OpenSoulChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OpenSoulChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenSoulChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OpenSoulChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OpenSoulChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OpenSoulChatTheme.assistantText)
            }
        }
    }
}
