...
@MainActor
struct OpenSoulChatComposer: View {
    @Bindable var viewModel: OpenSoulChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OpenSoulChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OpenSoulChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OpenSoulPendingAttachment.id)
                { (att: OpenSoulPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OpenSoulPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(OpenSoulChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenSoulChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OpenSoulChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OpenSoulChatTheme.subtleCard)
        .clipShape(Capsule())
    }
