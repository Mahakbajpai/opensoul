...
import Foundation
import Testing
@testable import OpenSoulChatUI
...
private final class TestChatTransport: @unchecked Sendable, OpenSoulChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenSoulChatHistoryPayload]
    private let sessionsResponses: [OpenSoulChatSessionsListResponse]

    private let stream: AsyncStream<OpenSoulChatTransportEvent>
    private let continuation: AsyncStream<OpenSoulChatTransportEvent>.Continuation

    init(
        historyResponses: [OpenSoulChatHistoryPayload],
        sessionsResponses: [OpenSoulChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpenSoulChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenSoulChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenSoulChatHistoryPayload(
        ...
        attachments _: [OpenSoulChatAttachmentPayload]) async throws -> OpenSoulChatSendResponse
    {
        ...
        return OpenSoulChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenSoulChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenSoulChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenSoulChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OpenSoulChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OpenSoulChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OpenSoulChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OpenSoulChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OpenSoulChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OpenSoulChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OpenSoulChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
