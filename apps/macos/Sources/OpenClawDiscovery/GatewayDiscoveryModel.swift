...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.opensoul", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in OpenSoulBonjour.gatewayServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "ai.opensoul.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = OpenSoulBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = OpenSoulBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = OpenSoulBonjour.wideAreaGatewayServiceDomain else { return false }
