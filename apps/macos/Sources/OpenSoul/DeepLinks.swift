...
private let deepLinkLogger = Logger(subsystem: "ai.opensoul", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "OpenSoul is paused", message: "Unpause OpenSoul to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run OpenSoul agent?", message: body) else { return }
        }
