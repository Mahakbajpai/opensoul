...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opensoul", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: OpenSoulNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [OpenSoulCapability.canvas.rawValue, OpenSoulCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(OpenSoulCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if OpenSoulLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(OpenSoulCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(OpenSoulCapability.camera.rawValue) {
        ...
        if capsSet.contains(OpenSoulCapability.location.rawValue) {
