...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: OpenSoulOAuthStore.AnthropicOAuthStatus = OpenSoulOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: OpenSoulOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = OpenSoulOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = OpenSoulOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. OpenSoul can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: OpenSoulOAuthStore.AnthropicOAuthStatus,
