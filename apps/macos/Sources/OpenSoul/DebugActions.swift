...
enum DebugActions {
    private static let verboseDefaultsKey = "opensoul.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "opensoul.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenSoulPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenSoul", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenSoulPaths.configURL
        guard
