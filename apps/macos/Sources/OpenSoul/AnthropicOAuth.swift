...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (OpenSoul token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: OpenSoulOAuthStore.AnthropicOAuthStatus = OpenSoulOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.opensoul", category: "anthropic-oauth")
...
enum OpenSoulOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let opensoulOAuthDirEnv = "OPENSOUL_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "OpenSoul OAuth token file not found"
            case .unreadableFile: "OpenSoul OAuth token file not readable"
            case .invalidJSON: "OpenSoul OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in OpenSoul OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "OpenSoul OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.opensoulOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".opensoul", isDirectory: true)
