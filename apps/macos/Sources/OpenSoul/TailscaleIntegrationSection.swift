...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.opensoul/opensoul.json. Prefer OPENSOUL_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.opensoul/opensoul.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.opensoul/opensoul.json. Restart the gateway to apply."
        }
