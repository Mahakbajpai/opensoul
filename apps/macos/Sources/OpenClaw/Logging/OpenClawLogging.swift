...
enum OpenSoulLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenSoulOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenSoulFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenSoulLogging.bootstrapIfNeeded()
        let label = OpenSoulLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenSoulOSLogHandler: LogHandler {
...
struct OpenSoulFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenSoulLogging.parseLabel(self.label)
