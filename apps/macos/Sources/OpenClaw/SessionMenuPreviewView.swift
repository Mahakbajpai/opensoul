...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.opensoul", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenSoulSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenSoulSessionPreviewEntry,
    ...
    private static func cache(payload: OpenSoulSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenSoulChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenSoulKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenSoulKit.AnyCodable]) -> [OpenSoulChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenSoulChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenSoulChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenSoulChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenSoulChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenSoulChatMessage) -> String? {
