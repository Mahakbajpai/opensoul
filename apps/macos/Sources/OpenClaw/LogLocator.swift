...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENSOUL_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/opensoul")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("opensoul-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("opensoul-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/opensoul/ (rolling or stdout), or nil if none exist.
