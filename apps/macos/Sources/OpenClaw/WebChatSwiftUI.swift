...
private let webChatSwiftLogger = Logger(subsystem: "ai.opensoul", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OpenSoulChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenSoulChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OpenSoulChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenSoulChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpenSoulGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    OpenSoulAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenSoulChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenSoulChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenSoulChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OpenSoulChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenSoulChatView>) -> NSViewController
    {
