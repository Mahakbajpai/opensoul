...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opensoul", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpenSoulPeekabooBridgeServices?
    private static var opensoulSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("OpenSoul", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenSoulPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.opensoulSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.opensoulSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OpenSoulPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.opensoul.peekaboo")
