...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpenSoulProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenSoulProtocol.AnyCodable] = [
            "presence": .init([OpenSoulProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenSoulProtocol.AnyCodable(payloadMap)
