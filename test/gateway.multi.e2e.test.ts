...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opensoul-e2e-${name}-`));
  const configDir = path.join(homeDir, ".opensoul");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENSOUL_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OPENSOUL_GATEWAY_TOKEN: inst.gatewayToken,
        OPENSOUL_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENSOUL_GATEWAY_PORT: String(gwA.port),
          OPENSOUL_GATEWAY_TOKEN: gwA.gatewayToken,
          OPENSOUL_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPENSOUL_GATEWAY_PORT: String(gwB.port),
          OPENSOUL_GATEWAY_TOKEN: gwB.gatewayToken,
          OPENSOUL_GATEWAY_PASSWORD: "",
        }),
