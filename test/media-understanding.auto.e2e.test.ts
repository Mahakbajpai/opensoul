...
import type { OpenSoulConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("opensoul-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("opensoul-bin-sherpa-");
      const modelDir = await makeTempDir("opensoul-sherpa-model-");
      ...
      const cfg: OpenSoulConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("opensoul-bin-whispercpp-");
      const modelDir = await makeTempDir("opensoul-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("opensoul-bin-gemini-");
      ...
      const cfg: OpenSoulConfig = { tools: { media: { image: {} } } };
