...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENSOUL_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OPENSOUL_STATE_DIR = path.join(base, ".opensoul");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "opensoul-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".opensoul", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
