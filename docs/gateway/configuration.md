---
summary: "All configuration options for ~/.opensoul/opensoul.json with examples"
read_when:
...
OpenSoul reads an optional **JSON5** config from `~/.opensoul/opensoul.json` (comments + trailing commas allowed).

If the file is missing, OpenSoul uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.opensoul/workspace`). You usually only need a config to:
...
OpenSoul only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `opensoul doctor`, `opensoul logs`, `opensoul health`, `opensoul status`, `opensoul service`, `opensoul help`).
- Run `opensoul doctor` to see the exact issues.
- Run `opensoul doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `opensoul config set`. Keep a backup of `~/.opensoul/opensoul.json`.
...
```bash
opensoul gateway call config.get --params '{}' # capture payload.hash
...
{
  agents: {
    defaults: { workspace: "~/.opensoul/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@opensoul", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.opensoul/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.opensoul/workspace" }],
}
```
...
{ "$include": "/etc/opensoul/base.json5" } // absolute
...
```json5
// ~/.opensoul/clients/mueller/agents.json5
[
...
```json5
// ~/.opensoul/clients/mueller/broadcast.json5
{
...
OpenSoul reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, OpenSoul runs your login shell and imports only the missing expected keys (never overrides).
...
OpenSoul stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.opensoul/agents/<agentId>/agent/auth-profiles.json`)
...
- `~/.opensoul/agent/*` (migrated by `opensoul doctor` into `~/.opensoul/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `OPENSOUL_OAUTH_DIR`
- Agent dir (default agent root override): `OPENSOUL_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, OpenSoul imports `oauth.json` entries into `auth-profiles.json`.
...
If set, OpenSoul derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/opensoul/opensoul-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/opensoul/opensoul.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/opensoul/opensoul.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
...
- The legacy single-account Baileys auth dir is migrated by `opensoul doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@opensoul", "opensoul"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@opensoul"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.opensoul/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.opensoul/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.opensoul/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.opensoul/workspace-home" },
      { id: "work", workspace: "~/.opensoul/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `opensoul.json`).
...
OpenSoul starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["opensoul-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-opensoul",
...
OpenSoul starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `opensoul plugins install @opensoul/mattermost` (or `./extensions/mattermost` from a git checkout).
...
OpenSoul starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
OpenSoul spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, OpenSoul
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/opensoul" } },
}
```
...
When a file exceeds this limit, OpenSoul logs a warning and injects a truncated
...
message envelopes). If unset, OpenSoul uses the host timezone at runtime.
...
`[opensoul]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[opensoul]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[opensoul]"`, OpenSoul will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, OpenSoul generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.opensoul/settings/tts.json",
      elevenlabs: {
...
OpenSoul also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, OpenSoul currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `opensoul configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:opensoul`: all built-in OpenSoul tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.opensoul/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "opensoul-sandbox-browser:bookworm-slim",
          containerPrefix: "opensoul-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenSoul uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.opensoul/agents/<agentId>/agent/models.json` or by defining the same schema inside your
OpenSoul config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, OpenSoul writes/merges a `models.json` into
`~/.opensoul/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. OpenSoul uses
...
- Shortcut: `opensoul onboard --auth-choice opencode-zen`.
...
Shortcut: `opensoul onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `opensoul onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `opensoul onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `opensoul onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `opensoul onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `opensoul onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENSOUL_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.opensoul/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.opensoul/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.opensoul/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, OpenSoul stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.opensoul/skills` (workspace skills
...
from `~/.opensoul/extensions`, `<workspace>/.opensoul/extensions`, plus any
...
### `browser` (opensoul-managed browser)

OpenSoul can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for opensoul and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (OpenSoul.app menubar, or `opensoul gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/opensoul" }
...
- Examples: `"/ui"`, `"/opensoul"`, `"/apps/opensoul"`.
...
- `opensoul gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OPENSOUL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OPENSOUL_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. OpenSoul
...
- OpenSoul.app watches `~/.opensoul/opensoul.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.opensoul/opensoul.json` (or `OPENSOUL_CONFIG_PATH`) and applies changes automatically.
...
- `~/.opensoul/opensoul.json` (or `OPENSOUL_CONFIG_PATH`)
...
- `OPENSOUL_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `opensoul --dev …` → uses `~/.opensoul-dev` + shifts ports from base `19001`
- `opensoul --profile <name> …` → uses `~/.opensoul-<name>` (port via config/env/flags)
...
```bash
OPENSOUL_CONFIG_PATH=~/.opensoul/a.json \
OPENSOUL_STATE_DIR=~/.opensoul-a \
opensoul gateway --port 19001
```
...
Gmail helper config (used by `opensoul webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "opensoul@gmail.com",
...
- Set `OPENSOUL_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, OpenSoul defaults `serve.path` to `/` so
...
Default root: `~/.opensoul/workspace/canvas`  
Default port: `18793` (chosen to avoid the opensoul browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__opensoul__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__opensoul__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.opensoul/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `OPENSOUL_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.opensoul/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.opensoul/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.opensoul/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_opensoul-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `opensoul` (advertises `opensoul.local`). Override with `OPENSOUL_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_opensoul-gw._tcp` under `~/.opensoul/dns/` using the configured discovery domain (example: `opensoul.internal.`).
