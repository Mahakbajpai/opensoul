...
- Replaces the legacy `gateway` command. CLI entry point: `opensoul gateway`.
...
- Config hot reload watches `~/.opensoul/opensoul.json` (or `OPENSOUL_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__opensoul__/canvas/` from `~/.opensoul/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPENSOUL_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OPENSOUL_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OPENSOUL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `OPENSOUL_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.opensoul/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `OPENSOUL_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `OPENSOUL_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `OPENSOUL_CONFIG_PATH`
- unique `OPENSOUL_STATE_DIR`
...
- `health` — full health snapshot (same shape as `opensoul health --json`).
...
- If other gateway-like services are detected, the CLI warns unless they are OpenSoul profile services.
  ...
  - Cleanup: `opensoul gateway uninstall` (current service) and `opensoul doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `opensoul gateway install --force` to reinstall (profile/env/path changes).
...
OpenSoul installs a **systemd user service** by default on Linux/WSL2. We
...
`opensoul gateway install` writes the user unit. `opensoul doctor` audits the
...
Create `/etc/systemd/system/opensoul-gateway[-<profile>].service` (copy the unit above,
...
- `opensoul gateway health|status` — request health/status over the Gateway WS.
- `opensoul message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `opensoul agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `opensoul gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `opensoul gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `opensoul gateway` and the legacy TCP control port.
