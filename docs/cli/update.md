---
summary: "CLI reference for `opensoul update` (safe-ish source update + gateway auto-restart)"
read_when:
...
opensoul update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), OpenSoul also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/opensoul`, override with `OPENSOUL_GIT_DIR`),
...
8. Runs `opensoul doctor` as the final “safe update” check.
...
- `opensoul doctor` (offers to run update first on git checkouts)
