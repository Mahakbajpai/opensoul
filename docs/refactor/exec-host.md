...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.opensoul/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.opensoul/node.json` (node id + pairing token).
...
Path: `~/.opensoul/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.opensoul/exec-approvals.sock",
...
- Unix socket at `~/.opensoul/exec-approvals.sock` (0600).
