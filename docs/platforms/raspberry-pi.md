---
summary: "OpenSoul on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenSoul on a Raspberry Pi
  - Running OpenSoul on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenSoul on Raspberry Pi

## Goal

Run a persistent, always-on OpenSoul Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenSoul features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled opensoul

# Enable if not
sudo systemctl enable opensoul

# Start on boot
sudo systemctl start opensoul
```
...
```bash
# Check logs
journalctl -u opensoul --no-pager -n 100

# Common fix: rebuild
cd ~/opensoul  # if using hackable install
npm run build
sudo systemctl restart opensoul
```
