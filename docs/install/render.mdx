...
Deploy OpenSoul on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/opensoul/opensoul)
...
repository configures everything needed to run OpenSoul:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENSOUL_STATE_DIR
        value: /data/.opensoul
      - key: OPENSOUL_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENSOUL_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/opensoul`.
...
If you use the original OpenSoul repository, Render will not auto-deploy your OpenSoul. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenSoul, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenSoul host.
