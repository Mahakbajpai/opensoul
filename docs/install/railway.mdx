...
Deploy OpenSoul on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/opensoul` — Control UI
...
- `OPENSOUL_STATE_DIR=/data/.opensoul` (recommended)
- `OPENSOUL_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENSOUL_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenSoul state + workspace so you can migrate to another host without losing config or memory.
