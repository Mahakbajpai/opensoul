---
summary: "Chrome extension: let OpenSoul drive your existing Chrome tab"
read_when:
...
The OpenSoul Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate opensoul-managed Chrome profile.
...
OpenSoul then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenSoul release (npm package) as static files. There is no separate “build” step.

After upgrading OpenSoul:

- Re-run `opensoul browser extension install` to refresh the installed files under your OpenSoul state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenSoul ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Use it:

- CLI: `opensoul browser --browser-profile chrome tabs`
...
- Open the tab you want OpenSoul to control.
...
- `ON`: attached; OpenSoul can drive that tab.
...
Debugging: `opensoul sandbox explain`
...
`opensoul browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `opensoul browser extension install` first to copy the extension to a stable location under your OpenSoul state directory.
...
- **This is not isolated** like the dedicated opensoul-managed profile.
