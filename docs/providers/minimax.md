---
summary: "Use MiniMax M2.1 in OpenSoul"
read_when:
  - You want MiniMax models in OpenSoul
...
```bash
opensoul plugins enable minimax-portal-auth  # skip if already loaded.
opensoul gateway restart  # restart if gateway is already running
opensoul onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/opensoul/opensoul/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `opensoul.json`:
...
## Configure via `opensoul configure`

Use the interactive config wizard to set MiniMax without editing JSON:

1. Run `opensoul configure`.
2. Select **Model/auth**.
...
- Use `opensoul models list` and `opensoul models set minimax/MiniMax-M2.1` to switch.
...
- Running `opensoul configure` and selecting **MiniMax M2.1**, or
