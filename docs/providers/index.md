---
summary: "Model providers (LLMs) supported by OpenSoul"
read_when:
...
OpenSoul can use many LLM providers. Pick a provider, authenticate, then set the
...
1. Authenticate with the provider (usually via `opensoul onboard`).
