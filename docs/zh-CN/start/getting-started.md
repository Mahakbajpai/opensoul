...
- Node `>=22`
- `pnpm`（可选；如果从源代码构建则推荐）
- **推荐：**Brave Search API 密钥用于网页搜索。最简单的方式：`opensoul configure --section web`（存储 `tools.web.search.apiKey`）。参见 [Web 工具](/tools/web)。
...
- OAuth 凭证（旧版导入）：`~/.opensoul/credentials/oauth.json`
- 认证配置文件（OAuth + API 密钥）：`~/.opensoul/agents/<agentId>/agent/auth-profiles.json`
...
如果你还没有全局安装，从仓库通过 `pnpm opensoul ...` 运行新手引导步骤。`pnpm build` 也会打包 A2UI 资源；如果你只需要运行那个步骤，使用 `pnpm canvas:a2ui:bundle`。
...
如果 `opensoul health` 显示"未配置认证"，回到向导设置 OAuth/密钥认证——没有它智能体将无法响应。

提示：`opensoul status --all` 是最佳的可粘贴、只读调试报告。
健康探测：`opensoul health`（或 `opensoul status --deep`）向运行中的 Gateway 网关请求健康快照。
