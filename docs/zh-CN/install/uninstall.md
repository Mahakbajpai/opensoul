...
summary: 完全卸载 OpenSoul（CLI、服务、状态、工作区）
title: 卸载
x-i18n:
...
默认标签是 `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.opensoul.*` 可能仍然存在）：
...
如果你使用了配置文件，请将标签和 plist 名称替换为 `bot.molt.<profile>`。如果存在任何旧版 `com.opensoul.*` plist，请将其移除。
...
默认任务名称是 `OpenSoul Gateway`（或 `OpenSoul Gateway (<profile>)`）。
任务脚本位于你的状态目录下。
...
如果你使用了 `https://opensoul.ai/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g opensoul@latest` 安装的。
使用 `npm rm -g opensoul` 移除（或 `pnpm remove -g` / `bun remove -g`，如果你是用那种方式安装的）。
