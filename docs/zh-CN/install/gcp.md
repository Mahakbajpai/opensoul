---
read_when:
  - 你想在 GCP 上 24/7 运行 OpenSoul
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 OpenSoul Gateway 网关并持久化状态
title: GCP
x-i18n:
...
如果你想要"OpenSoul 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
OpenSoul 在 Docker 中运行，但 Docker 不是真实来源。
...
| WhatsApp 会话    | `/home/node/.opensoul/`           | 主机卷挂载    | 保留 QR 登录                |
