...
read_when:
  - 安装 OpenSoul
  - 你想从 GitHub 安装
summary: 安装 OpenSoul（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 opensoul、node-llama-cpp、sharp 等
pnpm add -g opensoul@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm opensoul ...` 运行仓库命令。
...
- 运行新手引导：`opensoul onboard --install-daemon`
- 快速检查：`opensoul doctor`
- 检查 Gateway 网关健康状态：`opensoul status` + `opensoul health`
- 打开仪表板：`opensoul dashboard`
...
```bash
# 显式 npm
curl -fsSL https://opensoul.ai/install.sh | bash -s -- --install-method npm
...
- `OPENSOUL_GIT_DIR=...`
- `OPENSOUL_GIT_UPDATE=0|1`
- `OPENSOUL_NO_PROMPT=1`
- `OPENSOUL_DRY_RUN=1`
- `OPENSOUL_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `opensoul`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `opensoul`）。
