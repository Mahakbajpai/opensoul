---
read_when:
  - 你想让 OpenSoul 在云 VPS 上 24/7 运行（而不是你的笔记本电脑）
  - 你想在自己的 VPS 上运行生产级、永久在线的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
  - 你在 Hetzner 或类似提供商上用 Docker 运行 OpenSoul
summary: 在廉价的 Hetzner VPS（Docker）上 24/7 运行 OpenSoul Gateway 网关，带持久状态和内置二进制文件
title: Hetzner
x-i18n:
...
# 在 Hetzner 上运行 OpenSoul（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 Hetzner VPS 上运行持久的 OpenSoul Gateway 网关，带持久状态、内置二进制文件和安全的重启行为。

如果你想要"约 $5 实现 OpenSoul 24/7"，这是最简单可靠的设置。
...
- 在主机上持久化 `~/.opensoul` + `~/.opensoul/workspace`（重启/重建后保留）
...
OpenSoul 在 Docker 中运行，但 Docker 不是事实来源。
所有长期状态必须在重启、重建和重启后保留。

| 组件             | 位置                              | 持久化机制    | 说明                        |
| ---------------- | --------------------------------- | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.opensoul/`           | 主机卷挂载    | 包括 `opensoul.json`、令牌  |
| 模型认证配置文件 | `/home/node/.opensoul/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.opensoul/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.opensoul/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.opensoul/`           | 主机卷挂载    | 保留二维码登录              |
| Gmail 密钥环     | `/home/node/.opensoul/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
