...
- CLI 辅助工具：`opensoul onboard`、`opensoul models list`、`opensoul models set <provider/model>`。

## 内置提供商（pi-ai 目录）

OpenSoul 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- 认证：`OPENAI_API_KEY`
- 示例模型：`openai/gpt-5.2`
- CLI：`opensoul onboard --auth-choice openai-api-key`
...
- CLI：`opensoul onboard --auth-choice token`（粘贴 setup-token）或 `opensoul models auth paste-token --provider anthropic`
...
- CLI：`opensoul onboard --auth-choice openai-codex` 或 `opensoul models auth login --provider openai-codex`
...
- CLI：`opensoul onboard --auth-choice opencode-zen`
...
- CLI：`opensoul onboard --auth-choice gemini-api-key`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`opensoul plugins enable google-antigravity-auth`
  - 登录：`opensoul models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`opensoul plugins enable google-gemini-cli-auth`
  - 登录：`opensoul models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `opensoul.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
...
- 认证：`ZAI_API_KEY`
- 示例模型：`zai/glm-4.7`
- CLI：`opensoul onboard --auth-choice zai-api-key`
...
- CLI：`opensoul onboard --auth-choice ai-gateway-api-key`
...
```bash
opensoul plugins enable qwen-portal-auth
opensoul models auth login --provider qwen-portal --set-default
```
...
- CLI：`opensoul onboard --auth-choice synthetic-api-key`
...
```bash
opensoul onboard --auth-choice opencode-zen
opensoul models set opencode/claude-opus-4-5
