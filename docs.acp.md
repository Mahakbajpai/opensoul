...
This document describes how the OpenSoul ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`opensoul acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenSoul Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a OpenSoul Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `opensoul acp` over stdio.

Example config:

```bash
opensoul config set gateway.remote.url wss://gateway-host:18789
opensoul config set gateway.remote.token <token>
```

Example run:

```bash
opensoul acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
opensoul acp --session agent:main:main
opensoul acp --session agent:design:main
opensoul acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “OpenSoul ACP” to start a thread.

## Execution Model

- ACP client spawns `opensoul acp` and speaks ACP messages over stdio.
...
```bash
opensoul acp --session agent:main:main
opensoul acp --session-label "support inbox"
opensoul acp --reset-session
```
...
`opensoul acp` resolves the Gateway URL and auth from CLI flags or config:
