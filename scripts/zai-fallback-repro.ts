...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "opensoul.json");
  ...
  const sessionId =
    process.env.OPENSOUL_ZAI_FALLBACK_SESSION_ID ??
    process.env.OPENSOUL_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["opensoul", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["opensoul", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
