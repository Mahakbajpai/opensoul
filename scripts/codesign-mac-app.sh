...
APP_BUNDLE="${1:-dist/OpenSoul.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t opensoul-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t opensoul-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t opensoul-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/OpenSoul" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/OpenSoul" "$APP_ENTITLEMENTS"
fi
