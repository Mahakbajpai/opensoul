 ...
 * `opensoul update`. Use it to verify the completion installation flow
...
\`opensoul update\`, \`opensoul doctor\`, and \`opensoul onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenSoul Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`opensoul completion --install\` later to enable.`));
    return;
  }
