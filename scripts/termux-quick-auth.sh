...
SERVER="${OPENSOUL_SERVER:-${CLAWDBOT_SERVER:-l36}}"
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenSoul Re-Auth" -c "After getting key, run: ssh $SERVER '~/opensoul/scripts/mobile-reauth.sh'" --id opensoul-auth
        ;;
    *)
