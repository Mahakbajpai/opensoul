...
const logRunner = (message) => {
  if (env.OPENSOUL_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[opensoul] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["opensoul.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
