...
#
# Suggested cron: */30 * * * * /home/admin/opensoul/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenSoul notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opensoul-alerts)
...
STATE_FILE="$HOME/.opensoul/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenSoul if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opensoul
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenSoul to $NOTIFY_PHONE..."
            opensoul send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenSoul is down. Run: ssh l36 '~/opensoul/scripts/mobile-reauth.sh'" "urgent"
    exit 1
