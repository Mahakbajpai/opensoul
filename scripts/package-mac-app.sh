...
# Build and bundle OpenSoul into a minimal .app we can open.
# Outputs to dist/OpenSoul.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenSoul.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenSoul"
BUNDLE_ID="${BUNDLE_ID:-ai.opensoul.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opensoul/opensoul/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenSoul/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenSoulBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenSoulGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenSoul"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenSoul"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenSoul"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenSoul" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenSoul/Resources/OpenSoul.icns" "$APP_ROOT/Contents/Resources/OpenSoul.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenSoul/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenSoulKit resources"
OPENSOULKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenSoulKit_OpenSoulKit.bundle"
if [ -d "$OPENSOULKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenSoulKit_OpenSoulKit.bundle"
  cp -R "$OPENSOULKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenSoulKit_OpenSoulKit.bundle"
else
  echo "WARN: OpenSoulKit resource bundle not found at $OPENSOULKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenSoul"
killall -q OpenSoul 2>/dev/null || true
