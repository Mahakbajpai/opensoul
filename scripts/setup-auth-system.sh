#!/bin/bash
# Setup OpenSoul Auth Management System
...
echo "=== OpenSoul Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'opensoul-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenSoul message: Send warning via OpenSoul itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/opensoul-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/opensoul-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/opensoul-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now opensoul-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status opensoul-auth-monitor.timer"
echo ""
echo "Quick commands:"
