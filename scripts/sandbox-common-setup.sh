...
BASE_IMAGE="${BASE_IMAGE:-opensoul-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-opensoul-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=opensoul.sandbox=1)
NOTE
