...
IMAGE_NAME="opensoul-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="opensoul-net-e2e-$$"
GW_NAME="opensoul-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPENSOUL_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPENSOUL_SKIP_CHANNELS=1" \
	  -e "OPENSOUL_SKIP_GMAIL_WATCHER=1" \
	  -e "OPENSOUL_SKIP_CRON=1" \
	  -e "OPENSOUL_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
