...
ZIP=${1:?"Usage: $0 OpenSoul-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/opensoul/opensoul/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OpenSoul-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/opensoul/opensoul/releases/download/v${VERSION}/"}
