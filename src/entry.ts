...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.OPENSOUL_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.OPENSOUL_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.OPENSOUL_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[opensoul] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[opensoul] ${parsed.error}`);
    process.exit(2);
  }
