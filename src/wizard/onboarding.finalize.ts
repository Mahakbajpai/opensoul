...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.opensoul.ai/gateway/health",
          "https://docs.opensoul.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.opensoul.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.opensoul/opensoul.json (gateway.auth.token) or OPENSOUL_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("opensoul config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("opensoul doctor --generate-gateway-token")}`,
        "Web UI stores a copy in this browser's localStorage (opensoul.control.settings.v1).",
        `Open the dashboard anytime: ${formatCliCommand("opensoul dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control OpenSoul."
            : "Copy/paste this URL in a browser on this machine to control OpenSoul.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("opensoul dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.opensoul.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.opensoul.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ? [
          ...
          "Docs: https://docs.opensoul.ai/tools/web",
        ].join("\n")
      : [
          "If you want your agent to be able to search the web, you’ll need an API key.",
          "",
          "OpenSoul uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("opensoul configure --section web")}`,
  ...
  await prompter.note(
    'What now: https://opensoul.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control OpenSoul."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control OpenSoul.",
  );
