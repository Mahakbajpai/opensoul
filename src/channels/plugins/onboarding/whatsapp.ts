...
function setWhatsAppDmPolicy(cfg: OpenSoulConfig, dmPolicy: DmPolicy): OpenSoulConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpenSoulConfig, allowFrom?: string[]): OpenSoulConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OpenSoulConfig, selfChatMode: boolean): OpenSoulConfig {
...
async function detectWhatsAppLinked(cfg: OpenSoulConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so OpenSoul can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for OpenSoul" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("opensoul channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
