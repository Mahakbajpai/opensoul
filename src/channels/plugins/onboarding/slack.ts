...
function setSlackDmPolicy(cfg: OpenSoulConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenSoul";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenSoul`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/opensoul",
          description: "Send a message to OpenSoul",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: OpenSoulConfig, allowFrom: string[]): OpenSoulConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
