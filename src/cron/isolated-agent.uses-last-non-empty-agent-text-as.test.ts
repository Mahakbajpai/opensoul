...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "opensoul-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".opensoul", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenSoulConfig> = {},
): OpenSoulConfig {
  const base: OpenSoulConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "opensoul"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".opensoul", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
