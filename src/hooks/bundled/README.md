...
This directory contains hooks that ship with OpenSoul. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.opensoul.ai/hooks/soul-evil
...
Hooks can be configured in `~/.opensoul/opensoul.json`:
...
2. Restart gateway: `pkill -9 -f 'opensoul.*gateway' && pnpm opensoul gateway`
3. Enable the hook: `opensoul hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.opensoul.ai/hooks
