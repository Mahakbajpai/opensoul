...
homepage: https://docs.opensoul.ai/hooks#command-logger
metadata:
  {
    "opensoul":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.opensoul/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.opensoul/logs/commands.log ~/.opensoul/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/opensoul`:
   ```
   /home/username/.opensoul/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
