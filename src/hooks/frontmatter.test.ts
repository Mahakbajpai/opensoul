...
`;
    ...
    expect(parsed.opensoul.emoji).toBe("💾");
    expect(parsed.opensoul.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.opensoul.emoji).toBe("📝");
    expect(parsed.opensoul.events).toEqual(["command"]);
    expect(parsed.opensoul.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.opensoul.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"opensoul": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"opensoul": {"events": ["test"]}}');
  });
...
describe("resolveOpenSoulMetadata", () => {
  it("extracts opensoul metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenSoulMetadata(frontmatter);
  ...
  it("returns undefined when opensoul key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        opensoul: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenSoul" },
            { id: "npm", kind: "npm", package: "@opensoul/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@opensoul/hook");
  });
...
homepage: https://docs.opensoul.ai/hooks#session-memory
metadata:
  {
    "opensoul":
      {
...
`;
    ...
    const opensoul = resolveOpenSoulMetadata(frontmatter);
    expect(opensoul).toBeDefined();
    expect(opensoul?.emoji).toBe("💾");
    expect(opensoul?.events).toEqual(["command:new"]);
    expect(opensoul?.requires?.config).toEqual(["workspace.dir"]);
    expect(opensoul?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const opensoul = resolveOpenSoulMetadata(frontmatter);
    expect(opensoul?.emoji).toBe("disk");
    expect(opensoul?.events).toEqual(["command:new"]);
  });
});
