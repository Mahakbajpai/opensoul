...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPENSOUL_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("OpenSoul Canvas");
    } finally {
      await handler.close();
