...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.opensoulCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.opensoulCanvasA2UIAction &&
        typeof window.opensoulCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.opensoulSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("opensoul:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OpenSoul node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.opensoulSendUserAction === "function"
        ? window.opensoulSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENSOUL_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OPENSOUL_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OpenSoul Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
