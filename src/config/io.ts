...
import type { OpenSoulConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareOpenSoulVersions } from "./version.js";
...
function coerceConfig(value: unknown): OpenSoulConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenSoulConfig;
}
...
function stampConfigVersion(cfg: OpenSoulConfig): OpenSoulConfig {
...
function warnIfConfigFromFuture(cfg: OpenSoulConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenSoulVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenSoul (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OpenSoulConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenSoulConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OpenSoulConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenSoulConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenSoulConfig) {
    clearConfigCache();
...
// module scope. `OPENSOUL_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENSOUL_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENSOUL_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OpenSoulConfig {
...
export async function writeConfigFile(cfg: OpenSoulConfig): Promise<void> {
  clearConfigCache();
