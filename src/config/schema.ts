...
import { OpenSoulSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof OpenSoulSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when OpenSoul writes the config.",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $OPENSOUL_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.opensoul/memory/{agentId}.sqlite).",
  ...
  "memory.backend": 'Memory backend ("builtin" for OpenSoul embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `opensoul plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.opensoul/extensions/<id>).",
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = OpenSoulSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "OpenSoulConfig";
