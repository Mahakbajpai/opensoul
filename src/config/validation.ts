...
import type { OpenSoulConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenSoulConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpenSoulConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenSoulSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenSoulConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenSoulConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpenSoulConfig)),
    ),
  };
}
