...
const ETC_OPENSOUL_DIR = path.join(ROOT_DIR, "etc", "opensoul");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "opensoul.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOpenSoulPath(...parts: string[]) {
  return path.join(ETC_OPENSOUL_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOpenSoulPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "opensoul.json"))).toEqual({
      shared: true,
    });
  });
});
