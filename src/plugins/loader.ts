...
import { discoverOpenSoulPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpenSoulPluginDefinition;
  register?: OpenSoulPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpenSoulPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpenSoulPluginDefinition;
...
export function loadOpenSoulPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOpenSoulPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "opensoul/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OpenSoulPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as OpenSoulPluginModule;
    } catch (err) {
