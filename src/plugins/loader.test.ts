...
import { loadOpenSoulPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OPENSOUL_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `opensoul-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "opensoul.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENSOUL_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpenSoulPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPENSOUL_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadOpenSoulPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@opensoul/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        opensoul: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
