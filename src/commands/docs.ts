...
const SEARCH_TOOL = "https://docs.opensoul.ai/mcp.SearchOpenSoul";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.opensoul.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('opensoul docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.opensoul.ai/");
      runtime.log(`Search: ${formatCliCommand('opensoul docs "your query"')}`);
    }
    return;
  }
