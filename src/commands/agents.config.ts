...
export function listAgentEntries(cfg: OpenSoulConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenSoulConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenSoulConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenSoulConfig): AgentSummary[] {
