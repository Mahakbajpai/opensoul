...
beforeEach(() => {
  ...
  resolveOpenSoulPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENSOUL_STATE_DIR;
  originalUpdateInProgress = process.env.OPENSOUL_UPDATE_IN_PROGRESS;
  process.env.OPENSOUL_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opensoul-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENSOUL_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENSOUL_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENSOUL_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpenSoulPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenSoulPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "opensoul-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPENSOUL_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENSOUL_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/opensoul.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPENSOUL_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
