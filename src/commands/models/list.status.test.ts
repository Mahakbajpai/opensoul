...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenSoulAgentDir: vi.fn().mockReturnValue("/tmp/opensoul-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/opensoul-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/opensoul-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenSoulAgentDir: mocks.resolveOpenSoulAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenSoulAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/opensoul-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/opensoul-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/opensoul-agent-custom");
