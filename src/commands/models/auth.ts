...
import { readConfigFileSnapshot, type OpenSoulConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpenSoulConfig, model: string): OpenSoulConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("opensoul plugins install")}\`.`,
    );
  }
