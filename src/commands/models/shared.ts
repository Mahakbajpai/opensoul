...
export async function updateConfig(
  mutator: (cfg: OpenSoulConfig) => OpenSoulConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenSoulConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenSoulConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("opensoul agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
