...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenSoulAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("opensoul models auth setup-token")}\` or \`${formatCliCommand("opensoul configure")}\`.`
          : `Run \`${formatCliCommand("opensoul configure")}\` or set an API key env var.`;
