...
function resolveMode(cfg: OpenSoulConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenSoul doctor");

  const root = await resolveOpenSoulPackageRoot({
  ...
  let cfg: OpenSoulConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("opensoul configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("opensoul config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("opensoul setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("opensoul doctor --fix")}" to apply changes.`);
  }
