...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenSoul status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("opensoul security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("opensoul security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.opensoul.ai/faq");
  runtime.log("Troubleshooting: https://docs.opensoul.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("opensoul status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("opensoul logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("opensoul status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("opensoul gateway probe")}`);
  }
}
