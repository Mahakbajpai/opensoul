...
import type { OpenSoulConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenSoulConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("opensoul security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("opensoul configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("opensoul config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("opensoul doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "opensoul config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("opensoul config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
