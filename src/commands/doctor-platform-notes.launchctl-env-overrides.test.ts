...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENSOUL_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENSOUL_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENSOUL_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENSOUL_GATEWAY_PASSWORD");
  });
