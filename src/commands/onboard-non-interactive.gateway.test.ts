...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OPENSOUL_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "opensoul");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OPENSOUL_STATE_DIR = stateDir;
    process.env.OPENSOUL_CONFIG_PATH = path.join(stateDir, "opensoul.json");
