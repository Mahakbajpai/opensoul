...
const requireAgentDir = () => {
  const agentDir = process.env.OPENSOUL_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENSOUL_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OPENSOUL_STATE_DIR;
  const previousAgentDir = process.env.OPENSOUL_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.OPENSOUL_AGENT_DIR;
    } else {
      process.env.OPENSOUL_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OPENSOUL_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENSOUL_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-oauth-"));
    process.env.OPENSOUL_STATE_DIR = tempStateDir;
    process.env.OPENSOUL_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENSOUL_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENSOUL_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-minimax-"));
    process.env.OPENSOUL_STATE_DIR = tempStateDir;
    process.env.OPENSOUL_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENSOUL_AGENT_DIR;
