...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.OPENSOUL_GATEWAY_PASSWORD,
    disableConfigCache: process.env.OPENSOUL_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("OPENSOUL_STATE_DIR", prev.stateDir);
  restoreEnvVar("OPENSOUL_CONFIG_PATH", prev.configPath);
  restoreEnvVar("OPENSOUL_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("OPENSOUL_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("OPENSOUL_SKIP_CRON", prev.skipCron);
  restoreEnvVar("OPENSOUL_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("OPENSOUL_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("OPENSOUL_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("OPENSOUL_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.OPENSOUL_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "opensoul.json");
  process.env.HOME = tempHome;
  process.env.OPENSOUL_STATE_DIR = tempHome;
  process.env.OPENSOUL_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("opensoul-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("opensoul-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("opensoul-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("opensoul-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("opensoul-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("opensoul-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
