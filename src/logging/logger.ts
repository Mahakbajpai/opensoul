...
export const DEFAULT_LOG_DIR = "/tmp/opensoul";
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "opensoul.log"); // legacy single-file path

const LOG_PREFIX = "opensoul";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OpenSoulConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => OpenSoulConfig;
      };
