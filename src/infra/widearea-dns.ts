...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENSOUL_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*opensoul-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "opensoul";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "opensoul");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "opensoul-gw");
  ...
  records.push(`_opensoul-gw._tcp IN PTR ${instanceLabel}._opensoul-gw._tcp`);
  records.push(`${instanceLabel}._opensoul-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._opensoul-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; opensoul-content-hash: ${contentHash}\n${contentBody}`;
}
