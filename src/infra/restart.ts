...
export function triggerOpenSoulRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENSOUL_SYSTEMD_UNIT,
        process.env.OPENSOUL_PROFILE,
      );
  ...
  const label =
    process.env.OPENSOUL_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENSOUL_PROFILE);
