...
import { resolveOpenSoulPackageRoot, resolveOpenSoulPackageRootSync } from "./opensoul-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOpenSoulPackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "opensoul" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveOpenSoulPackageRootSync({
    argv1,
  ...
  if (argv1Dir) {
    // opensoul.mjs or dist/<bundle>.js
