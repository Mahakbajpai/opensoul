...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "opensoul.mjs");
    ...
    const doctorStep = await runStep(
      step("opensoul doctor", doctorArgv, gitRoot, { OPENSOUL_UPDATE_IN_PROGRESS: "1" }),
    );
