...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "opensoul.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (OpenSoul)",
    ...
    expect(txt).toContain(`$ORIGIN opensoul.internal.`);
    ...
    expect(txt).toContain(`_opensoul-gw._tcp IN PTR studio-london._opensoul-gw._tcp`);
    expect(txt).toContain(`studio-london._opensoul-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (OpenSoul)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/opensoul`);
  });
