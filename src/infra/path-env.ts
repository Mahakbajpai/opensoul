...
function candidateBinDirs(opts: EnsureOpenSoulPathOpts): string[] {
  ...
  // Bundled macOS app: `opensoul` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "opensoul");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/opensoul` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "opensoul"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `opensoul` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenSoulCliOnPath(opts: EnsureOpenSoulPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENSOUL_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPENSOUL_PATH_BOOTSTRAPPED = "1";
