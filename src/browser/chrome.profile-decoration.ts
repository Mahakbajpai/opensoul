...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".opensoul-profile-decorated");
}
 ...
 */
export function decorateOpenSoulProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENSOUL_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENSOUL_BROWSER_COLOR).toUpperCase();
