import type { BrowserConfig, BrowserProfileConfig, OpenSoulConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENSOUL_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENSOUL_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "opensoul" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENSOUL_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENSOUL_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenSoul browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for opensoul/opensoul CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenSoulConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENSOUL_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "opensoul";
