...
declare const __OPENSOUL_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPENSOUL_VERSION__ === "string" && __OPENSOUL_VERSION__) ||
  process.env.OPENSOUL_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `opensoul --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.OPENSOUL_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.OPENSOUL_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[opensoul] Gateway daemon failed:",
