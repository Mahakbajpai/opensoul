...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPENSOUL_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPENSOUL_STATE_DIR = path.join(home, ".opensoul");
    ...
    await fs.mkdir(path.join(home, ".opensoul"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.opensoul${path.sep}media`);
