...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): OpenSoulConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-workspace-"));
    try {
