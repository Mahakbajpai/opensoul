...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-embedded-agent-"));
...
const ensureModels = (cfg: OpenSoulConfig) => ensureOpenSoulModelsJson(cfg, agentDir) as unknown;
