...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opensoul.sandbox=1");
  args.push("--label", `opensoul.sessionKey=${params.scopeKey}`);
  args.push("--label", `opensoul.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `opensoul.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("opensoul.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opensoul sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opensoul sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opensoul sandbox recreate --all");
}
