...
import { createOpenSoulTools } from "./opensoul-tools.js";
import { __testing, createOpenSoulCodingTools } from "./pi-tools.js";
...
describe("createOpenSoulCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenSoulTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenSoulCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "opensoul-outside.txt");
