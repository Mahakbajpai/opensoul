...
import { createOpenSoulCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("opensoul-ws-", async (workspaceDir) => {
      await withTempDir("opensoul-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenSoulCodingTools({ workspaceDir });
    ...
    await withTempDir("opensoul-ws-", async (workspaceDir) => {
      await withTempDir("opensoul-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello opensoul");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("opensoul-ws-", async (workspaceDir) => {
      const tools = createOpenSoulCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("opensoul-ws-", async (workspaceDir) => {
      await withTempDir("opensoul-override-", async (overrideDir) => {
        const tools = createOpenSoulCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("opensoul-sandbox-", async (sandboxDir) => {
      await withTempDir("opensoul-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "opensoul-sbx-test",
        ...
        const tools = createOpenSoulCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
