...
function resolveExecConfig(cfg: OpenSoulConfig | undefined) {
...
export function createOpenSoulCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createOpenSoulReadTool(freshReadTool)];
    }
