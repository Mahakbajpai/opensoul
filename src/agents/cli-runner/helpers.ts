...
/**
 * Cleanup suspended OpenSoul CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpenSoulConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-cli-images-"));
  const paths: string[] = [];
