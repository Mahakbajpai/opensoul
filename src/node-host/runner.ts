...
import { ensureOpenSoulCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.OPENSOUL_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OPENSOUL_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpenSoulCliOnPath({ pathEnv: process.env.PATH ?? "" });
