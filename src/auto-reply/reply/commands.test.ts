...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-commands-"));
...
function buildParams(commandBody: string, cfg: OpenSoulConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
