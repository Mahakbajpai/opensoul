...
export function isCommandEnabled(cfg: OpenSoulConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenSoulConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenSoulConfig),
...
export function getCommandDetection(_cfg?: OpenSoulConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenSoulConfig) {
