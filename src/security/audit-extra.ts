...
import type { OpenSoulConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: OpenSoulConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: OpenSoulConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPENSOUL_STATE_DIR on a local-only volume and re-run "${formatCliCommand("opensoul security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: OpenSoulConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer OPENSOUL_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: OpenSoulConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: OpenSoulConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: OpenSoulConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: OpenSoulConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OpenSoulConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OpenSoulConfig): boolean {
...
function isBrowserEnabled(cfg: OpenSoulConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OpenSoulConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: OpenSoulConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function listWorkspaceDirs(cfg: OpenSoulConfig): string[] {
...
export async function collectPluginsCodeSafetyFindings(params: {
  ...
  const entries = await fs.readdir(extensionsDir, { withFileTypes: true }).catch((err) => {
    findings.push({
      ...
      remediation:
        "Check file permissions and plugin layout, then rerun `opensoul security audit --deep`.",
    });
    return [];
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all opensoul.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your OpenSoul extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (entry.skill.source === "opensoul-bundled") {
        continue;
      }
      ...
      const summary = await scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `opensoul security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
