...
export function resolveHooksConfig(cfg: OpenSoulConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-opensoul-token"] === "string"
      ? req.headers["x-opensoul-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
