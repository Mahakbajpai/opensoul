...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPENSOUL_STATE_DIR;
  previousConfigPath = process.env.OPENSOUL_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPENSOUL_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPENSOUL_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPENSOUL_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.OPENSOUL_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENSOUL_STATE_DIR = path.join(tempHome, ".opensoul");
  delete process.env.OPENSOUL_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENSOUL_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENSOUL_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENSOUL_SKIP_CANVAS_HOST = "1";
  process.env.OPENSOUL_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "opensoul-test-no-bundled-extensions")
    : "opensoul-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "opensoul-test-"))
    : path.join(tempHome, ".opensoul-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OPENSOUL_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPENSOUL_CONFIG_PATH;
    } else {
      process.env.OPENSOUL_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPENSOUL_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPENSOUL_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPENSOUL_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENSOUL_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPENSOUL_SKIP_CANVAS_HOST;
    } else {
      process.env.OPENSOUL_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.OPENSOUL_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.OPENSOUL_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.OPENSOUL_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENSOUL_GATEWAY_TOKEN = fallbackToken;
  }
