...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.OPENSOUL_CONFIG_PATH;
  const prevDisableCache = process.env.OPENSOUL_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "opensoul-canvas-auth-test-"));
  const configPath = path.join(dir, "opensoul.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.OPENSOUL_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.OPENSOUL_DISABLE_CONFIG_CACHE;
    } else {
      process.env.OPENSOUL_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
