...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.OPENSOUL_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "opensoul");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.opensoul-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".opensoul");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "opensoul.json");
      ...
      try {
        ...
        process.env.OPENSOUL_CONFIG_PATH = prev.configPath;
        process.env.OPENSOUL_GATEWAY_TOKEN = prev.token;
        process.env.OPENSOUL_SKIP_CHANNELS = prev.skipChannels;
        process.env.OPENSOUL_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.OPENSOUL_SKIP_CRON = prev.skipCron;
        process.env.OPENSOUL_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.OPENSOUL_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opensoul-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.OPENSOUL_STATE_DIR = prev.stateDir;
      process.env.OPENSOUL_CONFIG_PATH = prev.configPath;
      process.env.OPENSOUL_GATEWAY_TOKEN = prev.token;
      process.env.OPENSOUL_SKIP_CHANNELS = prev.skipChannels;
      process.env.OPENSOUL_SKIP_GMAIL_WATCHER = prev.skipGmail;
