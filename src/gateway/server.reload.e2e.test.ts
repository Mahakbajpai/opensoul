...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPENSOUL_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPENSOUL_SKIP_GMAIL_WATCHER;
    process.env.OPENSOUL_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPENSOUL_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPENSOUL_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENSOUL_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
