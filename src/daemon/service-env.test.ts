...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENSOUL_GATEWAY_PORT).toBe("18789");
    expect(env.OPENSOUL_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENSOUL_SERVICE_MARKER).toBe("opensoul");
    expect(env.OPENSOUL_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENSOUL_SERVICE_VERSION).toBe("string");
    expect(env.OPENSOUL_SYSTEMD_UNIT).toBe("opensoul-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENSOUL_LAUNCHD_LABEL).toBe("ai.opensoul.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENSOUL_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENSOUL_SYSTEMD_UNIT).toBe("opensoul-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENSOUL_LAUNCHD_LABEL).toBe("ai.opensoul.work");
    }
  });
});
