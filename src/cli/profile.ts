...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.opensoul${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.OPENSOUL_PROFILE = profile;

  const stateDir = env.OPENSOUL_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.OPENSOUL_STATE_DIR?.trim()) {
    env.OPENSOUL_STATE_DIR = stateDir;
  }

  if (!env.OPENSOUL_CONFIG_PATH?.trim()) {
    env.OPENSOUL_CONFIG_PATH = path.join(stateDir, "opensoul.json");
  }

  if (profile === "dev" && !env.OPENSOUL_GATEWAY_PORT?.trim()) {
    env.OPENSOUL_GATEWAY_PORT = "19001";
  }
}
