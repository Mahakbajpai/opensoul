...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opensoul-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENSOUL_NO_RESPAWN: "1",
      OPENSOUL_STATE_DIR: stateDir,
      OPENSOUL_SKIP_CHANNELS: "1",
      OPENSOUL_SKIP_GMAIL_WATCHER: "1",
      OPENSOUL_SKIP_CRON: "1",
      OPENSOUL_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENSOUL_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "opensoul-entry-bootstrap.mjs");
