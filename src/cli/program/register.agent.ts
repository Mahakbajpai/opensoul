...
${formatHelpExamples([
  ['opensoul agent --to +15555550123 --message "status update"', "Start a new session."],
  ['opensoul agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'opensoul agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'opensoul agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['opensoul agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'opensoul agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.opensoul.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.opensoul.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['opensoul agents set-identity --agent main --name "OpenSoul" --emoji "🦞"', "Set name + emoji."],
  ["opensoul agents set-identity --agent main --avatar avatars/opensoul.png", "Set avatar path."],
  [
    "opensoul agents set-identity --workspace ~/.opensoul/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "opensoul agents set-identity --identity-file ~/.opensoul/workspace/IDENTITY.md --agent main",
